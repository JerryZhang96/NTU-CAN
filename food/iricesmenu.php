    ...
    <nav>
      ...
      <ul class="nav-links">
        ...
        if (isset($_SESSION['userId'])) {
          ...
          <button type="submit" class="auth-btn" name="logoutbtn">Logout</button>
      </form>';
      } else {
        echo '<li class="auth"><a href="../login.php" class="auth-btn">Login</a></li>
        <li class="auth"><a href="../signup.php" class="auth-btn">Register</a></li>';
      } 
      ?>
      </ul>
    </nav>
        <button style="margin-left: 120px;" class="btn" onclick="window.history.back()">Go Back</button>
        <h1>Indian Rice Menu</h1>
            <?php
                $result=mysqli_query($conn, "SELECT * FROM products LIMIT 52,3") or die("SELECT * FROM products"."<br/><br/>".mysqli_error($conn));
